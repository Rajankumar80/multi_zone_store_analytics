...
# -----------------------------------------------
"""
camera_manager.py — Multi-process frame capture + grid display

Architecture
────────────
  [CaptureGroupProcess]  (one OS process per core group)
      └── [SingleCameraReader threads] ──► frame_queue (mp.Queue, JPEG bytes)

  Main process (core 0)
      ├── Pipeline (model.py)  reads frame_queue → produces zone_queue
      └── MultiCameraManager.display_streams()
              ├── drain thread  — empties zone_queue, stores latest JPEG bytes
              ├── decode thread — decodes JPEG → numpy, signals display-dirty
              └── display loop  — recomposes grid only when dirty, calls imshow

Performance fixes applied (no logic changes)
────────────────────────────────────────────
  FIX 1 — Unlimited drain with latest-per-camera semantics
      Old: drained max len(cams)*2 items per tick → queue built backlog → stale frames
      New: dedicated drain thread empties the entire queue every tick, keeping only
           the single newest JPEG payload per camera.

  FIX 2 — JPEG decode moved off the display thread
      Old: display loop decoded 6 JPEG frames per tick on core 0 → blocked waitKey
      New: dedicated decode thread (also core 0 but interleaved) decodes the latest
           snapshot and writes decoded numpy arrays into a lock-protected dict.
           Display thread only reads already-decoded arrays — zero decode cost.

  FIX 3 — Grid recomposed only when new frames arrived (_dirty flag)
      Old: _compose_grid (6× resize + hstack + vstack) ran every single tick
      New: _decode_version counter increments each time decode completes.
           Display caches the version it last rendered; skips recompose when equal.
           Cached grid numpy is reused, cutting display CPU by ~80% when pipeline
           is slower than the display rate.

  FIX 4 — Clean shutdown (no semaphore leaks)
      Old: stop() called proc.kill() then os._exit(0).  os._exit skips Python
           cleanup, leaving all mp.Queue semaphores unregistered → 15 leaked
           semaphore warnings.
      New: stop() calls proc.terminate() + proc.join(timeout=2) then sys.exit(0).
           sys.exit raises SystemExit which lets Python's atexit handlers run and
           mp resource_tracker clean up semaphores properly.
"""
...
def _jpeg_dec(data: bytes) -> "np.ndarray | None":
    """Decode JPEG bytes → BGR numpy array. Returns None on failure."""
    if not data:
        return None
...
def resize_for_preview(img: np.ndarray, target_w: int, target_h: int) -> np.ndarray:
    """
    Letterbox-resize to (target_w × target_h) using INTER_LINEAR.
    INTER_LINEAR is ~4× cheaper than INTER_AREA — keeps display thread light.
    """
    h, w    = img.shape[:2]
    scale   = min(target_w / w, target_h / h)
    new_w   = int(w * scale)
...
# ─── STAGE 0 — Capture workers (threads inside a sub-process) ────────────────

class SingleCameraReader(threading.Thread):
    """
    One thread per camera. grab()/retrieve() async decode loop.
    Frames are JPEG-compressed before queuing (~30× smaller than raw numpy).
    Auto-reconnects on RTSP drop.
    """

    def __init__(self, cam_name: str, source: str,
                 frame_queue: mp.Queue, fps: int) -> None:
        super().__init__(daemon=True, name=f"thread-{cam_name}")
        self.cam_name    = cam_name
        self.source      = source
        self.frame_queue = frame_queue
    ...
    def _push(self, frame: np.ndarray) -> None:
        ...
        if not ok:
            return
        item = {"cam": self.cam_name, "frame": buf.tobytes()}
        # Drop oldest so this thread is never blocked
        if self.frame_queue.full():
            try:
                self.frame_queue.get_nowait()
            except Exception:
    ...
    def run(self) -> None:
        ...
        while True:
            cap = cv2.VideoCapture(self.source)
            if not cap.isOpened():
                print(f"[WARN] [{self.cam_name}] Cannot open source — retry 3 s")
                time.sleep(3)
                continue
            ...
            while True:
                ...
                if not grabbed:
                    if is_rtsp:
                        print(f"[WARN] [{self.cam_name}] RTSP drop — reconnecting")
                        break
                    else:
                        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                        continue
...
class CaptureGroupProcess(mp.Process):
    """
    Isolated OS process running one SingleCameraReader thread per camera.
    Pinned to *core_id*. Self-terminates if parent process dies.
    """

    def __init__(self, group_id: int, cameras: list,
                 frame_queue: mp.Queue, fps: int,
                 core_id: int, parent_pid: int) -> None:
        super().__init__(daemon=True, name=f"cap-group-{group_id}")
        self.group_id    = group_id
        self.cameras     = cameras
        self.frame_queue = frame_queue
    ...
    def run(self) -> None:
        ...
        def _watch() -> None:
            while True:
                try:
                    ppid = os.getppid()
                    if ppid != self.parent_pid or ppid == 1:
                        os._exit(1)   # intentional hard-kill for orphan prevention
                except Exception:
                    os._exit(1)
                time.sleep(1.0)

        threading.Thread(target=_watch, daemon=True, name="cap-watchdog").start()

        if self.core_id is not None:
            try:
                os.sched_setaffinity(0, {self.core_id})
                print(f"[INFO] [cap-group-{self.group_id}] Pinned to core {self.core_id}")
            except (AttributeError, OSError):
                pass

        threads = [
            SingleCameraReader(c["name"], c["source"], self.frame_queue, self.fps)
...
class MultiCameraManager:
    """
    Manages capture sub-processes and the display loop.

    Internal threading model (all on core 0, interleaved)
    ──────────────────────────────────────────────────────
    drain thread   — continuously empties zone_queue into _raw_latest dict
                     (stores only the newest JPEG bytes per camera).
                     Signals _decode_event when new data arrives.

    decode thread  — wakes on _decode_event, takes snapshot of _raw_latest,
                     decodes JPEG → numpy for each updated camera, writes into
                     _dec_frames / _dec_floors under _dec_lock, increments
                     _decode_version, signals _display_event.

    display loop   — wakes on _display_event (or at most every display tick),
                     reads _dec_frames / _dec_floors under _dec_lock,
                     recomposes grid ONLY if _decode_version changed since last
                     render, calls cv2.imshow, calls cv2.waitKey.
    """
    ...
    def __init__(self,
        ...
        self.display_scale = display_scale
        self._no_signal    = _make_no_signal(*img_size)
        self._no_map       = _make_no_signal(900, 750)

        # ── Capture process groups ────────────────────────────────────────
        self._capture_groups: list[CaptureGroupProcess] = []
        parent_pid = os.getpid()

        for i, chunk in enumerate(
            [cameras[j:j + cams_per_core] for j in range(0, len(cameras), cams_per_core)]
        ):
            self._capture_groups.append(CaptureGroupProcess(
                group_id    = i,
                cameras     = chunk,
                frame_queue = frame_queue,
                fps         = CAPTURE_FPS,
                core_id     = start_core_id + i,
                parent_pid  = parent_pid,
            ))

        # ── Display state ────────────────────────────────────────────────
        self.focused_cam:      str | None       = None
        self._cam_bboxes:      dict[str, tuple] = {}
        self._last_grid_shape: tuple | None     = None
        self._cached_grid:     np.ndarray | None = None  # FIX 3: cached grid

        # ── FIX 1+2: Drain thread → raw bytes; Decode thread → numpy ─────

        # Stage A: drain thread writes newest JPEG bytes per camera here
        self._raw_latest: dict[str, dict | None] = {n: None for n in self.cam_names}
        self._raw_lock   = threading.Lock()
        self._decode_event = threading.Event()   # drain → decode signal

        # Stage B: decode thread writes decoded numpy arrays here
        self._dec_frames:  dict[str, np.ndarray | None] = {n: None for n in self.cam_names}
        self._dec_floors:  dict[str, np.ndarray | None] = {n: None for n in self.cam_names}
        self._dec_lock     = threading.Lock()
        self._display_event = threading.Event()  # decode → display signal

        # FIX 3: version counter — incremented by decode thread
        # Display thread caches the last version it rendered and skips
        # _compose_grid when the version hasn't changed.
        self._decode_version: int = 0
        self._rendered_version: int = -1
    ...
    def start(self) -> None:
        ...
        print(f"[INFO] Starting {len(self.cameras)} camera(s) in "
              f"{len(self._capture_groups)} capture process(es)")
        for cg in self._capture_groups:
            cg.start()

        # Start background threads (daemon — auto-stop when main exits)
        threading.Thread(target=self._drain_worker,  daemon=True, name="drain").start()
        threading.Thread(target=self._decode_worker, daemon=True, name="decode").start()
    ...
    def stop(self) -> None:
        """
        FIX 4: Clean shutdown — terminate + join so Python cleanup runs.
        sys.exit(0) raises SystemExit which lets atexit/resource_tracker
        unregister all mp.Queue semaphores (eliminates the leaked-semaphore warning).
        """
        print("\n[CRITICAL] Terminating capture processes…")
        self._stop_flag.set()
        for cg in self._capture_groups:
            if cg.is_alive():
                cg.terminate()
        # Give each process up to 2 s to exit gracefully, then force-kill
        for cg in self._capture_groups:
            cg.join(timeout=2)
            if cg.is_alive():
                cg.kill()
        ...
        cv2.destroyAllWindows()
        print("[INFO] Terminated.")
        sys.exit(0)   # raises SystemExit → Python cleanup runs → no semaphore leaks

    # ── FIX 1: Drain thread — unlimited drain, latest-per-camera semantics ────

    def _drain_worker(self) -> None:
        """
        Continuously empties zone_queue.
        For each camera only the NEWEST item is kept — older frames are silently
        overwritten in _raw_latest so the decode thread always gets the freshest data.

        No fixed drain limit → queue backlog is fully cleared every cycle →
        display is always ≤1 decode-cycle behind the pipeline output.
        """
        while not self._stop_flag.is_set():
            updated = False

            # Drain every item currently in the queue in one tight loop
            while True:
                try:
                    item = self.zone_queue.get_nowait()
                except _queue.Empty:
                    break
                ...
                # Overwrite with newer item — only the latest matters
                with self._raw_lock:
                    self._raw_latest[cam] = item
                updated = True

            if updated:
                # Signal decode thread: new raw bytes are ready
                self._decode_event.set()

            # Small sleep prevents this thread from spinning at 100% on idle
            time.sleep(0.005)   # 200 Hz ceiling — far above any camera fps

    # ── FIX 2: Decode thread — JPEG decode off the display thread ────────────

    def _decode_worker(self) -> None:
        """
        Wakes whenever _decode_event is set by the drain thread.
        Takes a snapshot of _raw_latest (fast, just copies references),
        decodes JPEG bytes → numpy for every updated camera,
        writes results into _dec_frames / _dec_floors under _dec_lock,
        then increments _decode_version and signals _display_event.

        Moving all cv2.imdecode calls here means the display thread never
        blocks on JPEG decode — it only does a dict read under a short lock.
        """
        while not self._stop_flag.is_set():
            # Block until drain thread signals new data; timeout keeps
            # the stop_flag check alive even when no cameras are sending.
            fired = self._decode_event.wait(timeout=0.1)
            if not fired:
                continue
            self._decode_event.clear()

            # Snapshot raw bytes (fast — just copies dict references)
            with self._raw_lock:
                snapshot = dict(self._raw_latest)

            # Decode outside the lock — JPEG decode is the expensive step
            decoded_frames: dict = {}
            decoded_floors: dict = {}

            for cam, item in snapshot.items():
                if item is None:
                    continue
                ann = _jpeg_dec(item.get("annotated_frame", b""))
                flr = _jpeg_dec(item.get("floor_map",       b""))
                if ann is not None:
                    decoded_frames[cam] = ann
                if flr is not None:
                    decoded_floors[cam] = flr

            if not decoded_frames and not decoded_floors:
                continue

            # Write decoded arrays and bump version atomically
            with self._dec_lock:
                for cam, arr in decoded_frames.items():
                    self._dec_frames[cam] = arr
                for cam, arr in decoded_floors.items():
                    self._dec_floors[cam] = arr
    ...
    def _compose_grid(self, frames: dict) -> np.ndarray:
        """
        Build the 3-column annotated camera grid.
        Called ONLY when _decode_version has changed since last render (FIX 3).
        """
        COLS      = 3
        SEP_COL   = (50, 50, 50)
        SEP_THICK = 4
        ...
        TH        = BASE_H
        TW        = int(BASE_H * 16 / 9)
        v_sep     = np.full((TH, SEP_THICK, 3), SEP_COL, dtype=np.uint8)

        self._cam_bboxes.clear()
        grid_rows = []
        cur_y     = 0

        for i in range(0, len(self.cam_names), COLS):
            chunk     = self.cam_names[i:i + COLS]
            row_cells = []
            cur_x     = 0
            ...
            for j in range(COLS):
                if j < len(chunk):
                    cam   = chunk[j]
                    _f    = frames.get(cam)
                    frame = _f if _f is not None else self._no_signal
                    self._cam_bboxes[cam] = (cur_x, cur_y, cur_x + TW, cur_y + TH)
                else:
                    frame = self._no_signal

                row_cells.append(resize_for_preview(frame, TW, TH))
                cur_x += TW
                if j < COLS - 1:
                    row_cells.append(v_sep)
                    cur_x += SEP_THICK
            ...
            h_sep = np.full((SEP_THICK, row_img.shape[1], 3), SEP_COL, dtype=np.uint8)
            grid_rows.append(h_sep)
            cur_y += SEP_THICK

        return np.vstack(grid_rows[:-1]) if grid_rows else self._no_signal

    # ── Display loop ──────────────────────────────────────────────────────────

    def display_streams(self) -> None:
        """
        Main display loop (core 0).

        Reads pre-decoded numpy arrays from _dec_frames/_dec_floors (written
        by the decode thread).  Recomposes the grid only when _decode_version
        has advanced since the last render — otherwise reuses the cached grid.
        cv2.waitKey is the only blocking call and it blocks for at most
        `delay` ms, keeping the display at a steady DISPLAY_FPS.
        """
        cv2.namedWindow("Camera View", cv2.WINDOW_NORMAL)
        cv2.setMouseCallback("Camera View", self._mouse_cb)

        delay = max(1, int(1000 / DISPLAY_FPS))

        try:
            while True:
                # ── FIX 3: Only recompose when new frames decoded ──────────────
                current_version = self._decode_version   # read without lock (int read is atomic in CPython)
                new_data = (current_version != self._rendered_version)

                if new_data:
                    # Take a snapshot of decoded arrays (short lock, just references)
                    with self._dec_lock:
                        frames_snap = dict(self._dec_frames)
                        floors_snap = dict(self._dec_floors)
                    self._rendered_version = current_version
                else:
                    # No new decode — reuse whatever we had last tick
                    frames_snap = None
                    floors_snap = None

                # ── Compose or reuse display image ────────────────────────────
                if self.focused_cam is not None:
                    if new_data and frames_snap is not None:
                        _f    = frames_snap.get(self.focused_cam)
                        frame = _f if _f is not None else self._no_signal
                        _m    = floors_snap.get(self.focused_cam)
                        floor = _m if _m is not None else self._no_map

                        if self._last_grid_shape is not None:
                            h, w  = self._last_grid_shape[:2]
                            disp  = resize_for_preview(frame, w, h)
                        else:
                            disp  = frame

                        self._cached_grid = disp   # reuse if no new data next tick
                        cv2.namedWindow("Floor Map", cv2.WINDOW_NORMAL)
                        cv2.imshow("Floor Map", floor)

                    elif self._cached_grid is not None:
                        disp = self._cached_grid   # reuse cached
                    else:
                        disp = self._no_signal

                    cv2.imshow("Camera View", disp)

                else:
                    if new_data and frames_snap is not None:
                        # Recompose grid with fresh frames
                        grid = self._compose_grid(frames_snap)
                        self._last_grid_shape = grid.shape
                        self._cached_grid = grid   # cache for idle ticks

                    # Always show cached grid (either freshly built or reused)
                    if self._cached_grid is not None:
                        cv2.imshow("Camera View", self._cached_grid)
